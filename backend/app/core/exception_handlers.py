...
from app.core.exceptions import IncidentAssignmentError, IncidentNotFoundError, UserNotFoundError, IncidentStatusUpdateError
...
async def incident_status_update_handler(request: Request, exc: IncidentStatusUpdateError):
    return JSONResponse(status_code=403, content={"detial": str(exc)})